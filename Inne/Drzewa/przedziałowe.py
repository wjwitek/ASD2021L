class Node:
    def __init__(self, value):
        ...
        self.left = None
        self.right = None
        self.misc = 0  # any additional info needed for specific problem
        self.interval = None
...
class IntervalTree(Tree):
    def make_tree(self, array, start, end):
        # insert nodes
        if start > end:
            return
        mid = (end - start) // 2
        self.insert(array[mid], 0)
        self.make_tree(array, mid + 1, end)
        self.make_tree(array, start, mid - 1)
        # assign intervals to nodes

        def assign(node, a, b, prev):
            if node is None:
                pass
