...
Algorytm ma złożność O(n^2), bo taką złożoność mają zarówno quickselect jak i przepisanie elementów.
"""
...
def median(T):
    ...
    quick_select(T, (n**2 - n) // 2 + 1, n ** 2 - 1, n - 1, n)
    print(T)
    rewrite(T, (n**2 - n) // 2, (n**2 - n) // 2 + n - 1, n)


test = [[9, 6, 16, 19, 3], [15, 12, 6, 3, 5], [14, 16, 7, 13, 16], [8, 17, 11, 10, 20], [5, 6, 12, 3, 8]]
"""
for z in range(4):
    temp = [randint(1, 20) for _ in range(4)]
    test.append(temp)
print(test)
print(quick_select(test, 0, 15, 7, 4))
print(quick_select(test, 7, 15, 3, 4))
print(test)
rewrite(test, 6, 9, 4)
print(test)
"""
...
# [[11, 19, 16, 16], [7, 12, 14, 17], [9, 8, 10, 15], [3, 6, 9, 11]]
